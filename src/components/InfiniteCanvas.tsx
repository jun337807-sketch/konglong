...
function Canvas({ projectId, projectName, groupId, groupName, onBackToProjects }: { projectId: string; projectName: string; groupId: string; groupName: string; onBackToProjects: () => void }) {
  ...
  const handleSaveProject = useCallback(async (arg1?: any, arg2?: any) => {
    if (!initialDataLoaded) return; // Prevent overwriting with empty state before load
    
    // Check if the args are valid node/edge arrays, otherwise use state
    const currentNodes = Array.isArray(arg1) ? arg1 : nodes;
    const currentEdges = Array.isArray(arg2) ? arg2 : edges;

    const projectData = { nodes: currentNodes, edges: currentEdges };
    ...
    const loadData = async () => {
      try {
        ...
        if (res.success && res.projectData) {
          setNodes(res.projectData.nodes || []);
          setEdges(res.projectData.edges || []);
          setTimeout(() => setInitialDataLoaded(true), 100);
          return;
        }
      } catch(e) {
        console.warn('Fetch remote project fail');
      }
      setTimeout(() => setInitialDataLoaded(true), 100);
    };
    loadData();
  ...
  const runAIGeneration = useCallback(async (nodeId: string) => {
    ...
    try {
      if (node.data.type === 'text2image') {
        ...
        const imageUrl = await runImageGeneration(textInput);
...
export default function InfiniteCanvasWrapper({ onClose, renderTopRight, currentUser }: { onClose?: () => void, renderTopRight?: React.ReactNode, currentUser?: string }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[#0E0F11] flex flex-col animate-in fade-in duration-300">
      ...
      <ReactFlowProvider>
        <Canvas projectId={selectedProject.id} projectName={selectedProject.name} groupId={selectedGroup.id} groupName={selectedGroup.name} onBackToProjects={() => setSelectedProject(null)} />
      </ReactFlowProvider>
    </div>
  );
}
